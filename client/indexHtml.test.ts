...
const preferredSiteName = "맏춤법 검사기 - 리버스 맞춤법 검사기";
...
describe("index.html SEO metadata", () => {
  it("declares the preferred Google site name as WebSite structured data", () => {
    ...
    expect(website).toMatchObject({
      ...
      url: "https://reverse-spell-checker.site/",
    });
    ...
    expect(JSON.stringify(website?.alternateName)).not.toContain("vercel.app");
  });

  it("keeps all crawlable site name signals away from the hosting platform name", () => {
    const website = getJsonLdNodes().find(node => node["@type"] === "WebSite");

    expect(getMetaContent('property="og:site_name"')).toBe(preferredSiteName);
    expect(getMetaContent('name="application-name"')).toBe(preferredSiteName);
    expect(website?.name).toBe(preferredSiteName);
    expect(`${website?.name} ${website?.alternateName}`).not.toMatch(/vercel/i);
  });

  it("uses the hooked search description consistently", () => {
    const webapp = getJsonLdNodes().find(
  ...
  it("uses a stable crawlable favicon URL", () => {
    ...
    expect(
      existsSync(new URL("./public/favicon-48.png", import.meta.url))
    ).toBe(true);
    expect(existsSync(new URL("./public/favicon.svg", import.meta.url))).toBe(
      true
    );
    expect(
      existsSync(new URL("./public/apple-touch-icon.png", import.meta.url))
    ).toBe(true);
  });
});
